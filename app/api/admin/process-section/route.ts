...
export async function PUT(request: Request) {
  try {
    ...
    const body = (await request.json()) as { eyebrow?: string; title?: string };
    const eyebrow = String(body.eyebrow ?? "").trim();
    const title = String(body.title ?? "").trim();

    if (!eyebrow || !title) {
      return NextResponse.json(
        { error: "Process eyebrow and title are required." },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to update process section." },
      { status: 500 },
    );
  }
}

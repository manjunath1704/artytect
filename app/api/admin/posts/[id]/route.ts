...
import { POSTS_BUCKET, ensurePostsFeaturedImagesBucket, getAdminClient } from "@/lib/supabase/admin";
...
const uploadFeaturedImage = async (file: File, slug: string) => {
  await ensurePostsFeaturedImagesBucket();
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!title || !slug || !short_description || !content || !category || !author) {
      return NextResponse.json({ error: "Please fill in all required post fields." }, { status: 400 });
    }

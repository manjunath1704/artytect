        ...
        {items.length ? (
          <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
            <div className="grid gap-4">
              {items.map((item) => (
                <article key={item.id} className="grid gap-4 rounded-[32px] bg-white p-4 shadow-sm sm:grid-cols-[120px_1fr_auto]">
                  ...
                  <div className="flex items-center justify-between gap-4 sm:flex-col sm:items-end">
                    <div className="grid h-11 grid-cols-[40px_48px_40px] overflow-hidden rounded-full border border-[#ded3c8] bg-white">
                      <button type="button" aria-label="Decrease quantity" disabled={item.quantity === 1} onClick={() => updateQuantity(item.id, item.quantity - 1)} className="grid place-items-center disabled:opacity-35">
                        <Minus className="h-3.5 w-3.5" />
                      </button>
                      <output className="grid place-items-center border-x border-[#ded3c8] text-sm font-semibold">{item.quantity}</output>
                      <button type="button" aria-label="Increase quantity" onClick={() => updateQuantity(item.id, item.quantity + 1)} className="grid place-items-center">
